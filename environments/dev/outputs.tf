...
output "all_k8s_nodes" {
  description = "List of all Kubernetes nodes with names and IPs"
  value = concat(
    [for idx, ip in module.ec2.master_private_ips : "tth-k8s-pj-dev-master-${idx + 1}: ${ip}"],
    [for idx, ip in module.ec2.worker_private_ips : "tth-k8s-pj-dev-worker-${idx + 1}: ${ip}"]
  )
}